...
nyukin_yosoku.sql_zensha_list = "SELECT "
    ...
    + "billing_info.nyukin_yotei_date,"
    + "sum(pay_amount) as nyukin_yosoku,"
...
// 試験課別売上集計
nyukin_yosoku.sql_division_summary = "select entry_info.test_large_class_cd as division_cd,test_large_class.item_name as division,sum(pay_amount_total) as nyukin_yosoku"
...
nyukin_yosoku.sql_client_summary = "select entry_info.client_cd,client_list.name_1 as client,sum(pay_amount_total) as nyukin_yosoku from drc_sch.billing_info"
  + " left join drc_sch.entry_info ON(billing_info.entry_no = entry_info.entry_no) "
  + " left join drc_sch.client_list ON(entry_info.client_cd = client_list.client_cd) "
  + " where (billing_info.nyukin_yotei_date between $1 and $2) and billing_info.delete_check = 0";
  + " group by entry_info.client_cd,client_list.name_1";
