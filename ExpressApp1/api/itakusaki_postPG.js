//
// 委託先情報ダイアログからのPOSTを処理する
//
//var mysql = require('mysql');
var tools = require('../tools/tool');

// 得意先データのPOST
exports.itakusaki_post = function (req, res) {
	var client = client_check(req.body);
	if (client.client_cd === "") {
		res.send(client);
	} else {
		var sql = "SELECT client_cd FROM drc_sch.itakusaki_list WHERE client_cd = $1";
...
// 部署データのPOST
exports.itakusaki_division_post = function (req, res) {
	var division = client_check(req.body);
	if ((division.division_client_cd === "") || (division.division_cd === '')) {
		res.send(division);
	} else {
		var sql = "SELECT client_cd, division_cd FROM drc_sch.itakusaki_division_list WHERE client_cd = $1 AND division_cd = $2";
		pg.connect(connectionString, function (err, connection) {
			// SQL実行
...
// 担当者データのPOST
exports.itakusaki_person_post = function (req, res) {
...
var insertClient = function (connection, client, req, res) {
	...
	var updated = null;
	var updated_id = "";
	var sql = 'INSERT INTO drc_sch.itakusaki_list('
		+ 'client_cd,'
		+ 'name_1,'
		+ 'name_2,'
		+ "kana," // カナ
...
var updateClient = function (connection, client, req, res) {
	...
	var sql = 'UPDATE drc_sch.itakusaki_list SET '
		+ 'client_cd = $1,'
		+ 'name_1 = $2,'
		+ 'name_2 = $3,'
		+ "kana = $4,"		// カナ
...
var insertClientDivision = function (connection, division, req, res) {
	...
	var updated = null;
	var updated_id = "";
	var sql = 'INSERT INTO drc_sch.itakusaki_division_list('
		+ 'client_cd,'			// クライアントコード
		+ 'division_cd,'		// 部署コード
		+ 'name,'				// 部署名
		+ "kana,"				// カナ
...
var updateClientDivision = function (connection, division, req, res) {
	var updated = tools.getTimestamp("{0}/{1}/{2} {3}:{4}:{5}");
	var updated_id = req.session.uid;
	var sql = 'UPDATE drc_sch.itakusaki_division_list SET '
...
var insertClientPerson = function (connection, person, req, res) {
	...
	var updated = null;
	var updated_id = "";
	var sql = 'INSERT INTO drc_sch.itakusaki_person_list('
		+ 'client_cd,'			// クライアントコード
		+ 'division_cd,'		// 部署コード
		+ 'person_id,'			// 担当者ID
		+ 'name,'				// 担当者名
...
var updateClientPerson = function (connection, person, req, res) {
	var updated = tools.getTimestamp("{0}/{1}/{2} {3}:{4}:{5}");
	var updated_id = req.session.uid;
	var sql = 'UPDATE drc_sch.itakusaki_person_list SET '
		+ 'client_cd = $1,'		// クライアントCD
		+ 'division_cd = $2,'	// 部署CD
		+ 'person_id = $3,'		// 担当者CD
		+ 'name = $4,'			// 担当者名
