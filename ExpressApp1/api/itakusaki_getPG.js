// 委託先の取得
exports.itakusaki_get = function (req, res) {
...
exports.itakusaki_division_get = function (req, res) {
...
//	var sql_count = 'SELECT COUNT(*) AS cnt FROM drc_sch.client_list WHERE delete_check = $1 AND client_cd ~* \'^[' + index_str[index_no] + ']\'';
	var sql_count = 'SELECT COUNT(*) AS cnt FROM drc_sch.itakusaki_list WHERE delete_check = $1 AND kana ~* \'^[' + index_str[index_no] + ']\'';
	var sql = 'SELECT '
		+ 'client_cd,'
		+ 'name_1,'
		+ 'name_2,'
...
//		+ ' FROM drc_sch.client_list WHERE delete_check = $1 AND client_cd ~* \'^[' + index_str[index_no] + ']\' ORDER BY '
		+ ' FROM drc_sch.itakusaki_list WHERE delete_check = $1 AND kana ~* \'^[' + index_str[index_no] + ']\' ORDER BY '
...
var client_division_get_list = function (req, res) {
	var pg_params = getPagingParams(req);
	var sql_count = 'SELECT COUNT(*) AS cnt FROM drc_sch.itakusaki_division_list WHERE client_cd = $1 AND delete_check = $2';
	var sql = 'SELECT '
		+ 'client_cd,'
		+ 'division_cd,'
		+ 'name,'
		...
		+ "to_char(updated,'YYYY/MM/DD HH24:MI:SS') AS updated,"
		+ 'updated_id'
		+ ' FROM drc_sch.itakusaki_division_list WHERE client_cd = $1 AND delete_check = $2 ORDER BY '
		+ pg_params.sidx + ' ' + pg_params.sord
		+ ' LIMIT ' + pg_params.limit + ' OFFSET ' + pg_params.offset;
...
var client_person_get_list = function (req, res) {
	var pg_params = getPagingParams(req);
	var sql_count = 'SELECT COUNT(*) AS cnt FROM drc_sch.itakusaki_person_list WHERE client_cd = $1 AND division_cd = $2 AND delete_check = $3';
	var sql = 'SELECT '
		+ 'client_cd,'
		+ 'division_cd,'
		+ 'person_id,'
		...
		+ "to_char(updated,'YYYY/MM/DD HH24:MI:SS') AS updated,"
		+ 'updated_id'
		+ ' FROM drc_sch.itakusaki_person_list WHERE client_cd = $1 AND division_cd = $2 AND delete_check = $3 ORDER BY '
		+ pg_params.sidx + ' ' + pg_params.sord
		+ ' LIMIT ' + pg_params.limit + ' OFFSET ' + pg_params.offset;
...
var client_get_detail = function (req, res) {
	var sql_count = 'SELECT COUNT(*) AS cnt FROM drc_sch.itakusaki_list WHERE delete_check = $1';
	var sql = 'SELECT '
		+ 'client_cd,'
		+ 'name_1,'
		+ 'name_2,'
		...
		+ "to_char(updated,'YYYY/MM/DD HH24:MI:SS') AS updated,"
		+ 'updated_id'
		+ ' FROM drc_sch.itakusaki_list WHERE client_cd = $1';
	// SQL実行
	pg.connect(connectionString, function (err, connection) {
		// データを取得するためのクエリーを実行する
