...
uriageListPrintDivision.makeListTable = function() {
  ...
  $.each(rows,function(index,row) {
    var tr = $("<tr>" +
    "<td class='data_value border_up_left'>" + row.division + "</td>" +
    "<td class='data_value_num border_up_left_right'>" + row.uriage_sum + "</td>" +
    "</tr>"
    );
    // テーブル行を追加する
  ...
  // 親ウィンドウの#uriage_listを取得する
  var grid = window.opener.$("#uriage_list_detail");
  // グリッドのデータを取得する
  var rows = grid.getRowData(); // 全件取得する
  $.each(rows,function(index,row) {
    var tr = $("<tr>" +
    ...
    // テーブル行を追加する
    $("#uriage_list_detail_table").append(tr);
  });
