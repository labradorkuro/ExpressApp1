...
billingSummaryList.billingSummaryListPrintSub = function(sd, ed, keyword,shikenjo,pay_result,option, cw,target) {
  ...
  $.get(req_url,function(response) {
      ...
      for(var i = 0;i < response.records;i++) {
        ...
        "<td class='data_value border_up_left'>" + scheduleCommon.dateStringCheck(row.report_submit_date) + "</td>" +
  ...
  $.get(req_url,function(response) {
      ...
      for(var i = 0;i < response.records;i++) {
        var row = response.rows[i].cell;
          ...
          scheduleCommon.setQuotation(row.report_submit_date != null ? scheduleCommon.dateStringCheck(row.report_submit_date):"") + "," +
